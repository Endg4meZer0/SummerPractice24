...
const outputFilename = 'sheet_';

procedure printSheet(lp: list_prod; lo: list_ord; ls: list_ship; year: integer);
...
procedure printSheet(lp: list_prod; lo: list_ord; ls: list_ship; year: integer);
var f: text;
    prodCounter, ordCounter, shipCounter, i: integer;
    ...
    res: string;
begin
  assign(f, outputFilename + year.ToString() + '.txt');
  rewrite(f);
  ...
  while prodCounter <= lp.Count do begin
    ...
    while ordCounter < lo.Count do begin
      ...
      while (i < max_products) and not orderHasProduct do begin
        i := i + 1;
...
begin
  result := orderName;
  for i := 0 to 32 - orderName.Length do result := result + ' ';
  result := result + '    '; // те самые 4 пробела между полями
  res := cost.ToString();
  if res = '0' then res := '';
  result := result + res;
  for i := 0 to 10 - res.Length do result := result + ' ';
  result := result + '    ';
  res := orderAmount.ToString();
  result := result + res;
  for i := 0 to 8 - res.Length do result := result + ' ';
  result := result + '    ';
