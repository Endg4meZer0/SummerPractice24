...
begin
  ...
  if err_string = '' then begin
    ...
    if t_flag then append_err(err_string, 'НАИМЕНОВАНИЕ ЗАКАЗЧИКА: Обнаружены неприемлемые символы. Разрешены только русский и английский алфавит, цифры и нижнее подчёркивание.');
    ...
    while (s <> '') do begin
      ...
      if t_flag = false then begin
        ...
        val(t_s, t_i, t_err);
        if (t_err <> 0) or (t_i < 1) or (t_i > 99999999) or (t_s[1] = '+') or (t_s[1] = '-') then append_err(t_err_string, 'ТОВАР №' + i.ToString() + ': Кол-во товара должно являться целым числом из диапозона [1; 99999999], не должно быть букв и других символов, в том числе "+" и "-".');
