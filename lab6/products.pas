...
begin
  ...
  if err_string = '' then begin
    ...
    // Проверка на то, что первый символ, если и является буквой, то заглавной
    if t_s[1] in ['a'..'z', 'а'..'я'] then append_err(err_string, 'НАИМЕНОВАНИЕ ТОВАРА: Первый символ не должен являться строчной буквой.');
    ...
    t_i := pos('.', t_s);
    if (t_i < 1) or (t_s.Length - t_i <> 2) then append_err(err_string, 'СТОИМОСТЬ ТОВАРА: В стоимости товара обязательно должно присутствовать две цифры после точки, даже если дробное значение равно нулю (тогда стоимость должна иметь ".00" в конце).');
