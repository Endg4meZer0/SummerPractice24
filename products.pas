...
begin
  ...
  if err_string = '' then begin
    ...
    val(t_s, t_i, t_err);
    if (t_err <> 0) or (t_i > 99999999) or (t_s[1] = '+') or (t_s[1] = '-') then append_err(err_string, 'КОД ТОВАРА: Код товара должен являться целым числом из диапозона [1; 99999999], не должно быть букв и других символов, в том числе "+" и "-".');
    ...
    if t_flag then append_err(err_string, 'НАИМЕНОВАНИЕ ТОВАРА: Обнаружены неприемлемые символы. Разрешены только русский и английский алфавит, цифры и нижнее подчёркивание.');
    ...
    val(t_s, t_r, t_err);
    if (t_err <> 0) and (t_r <= 9999999.99) then append_err(err_string, 'СТОИМОСТЬ ТОВАРА: Стоимость товара должна являться вещественным числом максимум из 10 символов (7 цифр перед точкой, сама точка и две обязательных цифры после точки), не должно быть букв и других символов.');
    
    // Проверка на наличие точки и хотя бы одной цифры до и конкретно двух цифр после неё
    t_i := pos('.', t_s);
    if (t_i <= 1) or (t_s.Length - t_i <> 2) then append_err(err_string, 'СТОИМОСТЬ ТОВАРА: В стоимости товара обязательно должно присутствовать хотя бы одна цифра до и две цифры после точки, даже если дробное значение равно нулю (тогда стоимость должна иметь ".00" в конце).');
...
begin
  ...
  for i := 1 to pl.Count do begin
    if pl.List[i].name = p.name then append_err(err_string, 'НАИМЕНОВАНИЕ ТОВАРА: Произошёл конфликт в виде повтора наименования товара с наименованием другого уже зарегистрированного товара.');
  end;
