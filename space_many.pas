...
begin
  ...
  if s.Length = 0 then append_err(err_string, 'ФОРМАТ ДАННЫХ: Обнаружена пустая строка.')
    ...
    else if (count < 8) then append_err(err_string, 'ФОРМАТ ДАННЫХ: Слишком мало данных. Необходимо 6 обязательных полей (код заказа, наименование заказчика, номер телефона, день, месяц и год заказа), а также хотя бы один товар (2 поля: код и кол-во).')
